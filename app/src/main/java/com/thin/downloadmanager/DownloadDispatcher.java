...
public class DownloadDispatcher extends Thread {
    	...
    	while(true) {
    		try {
                ...
                if (mQuit) {
                    if(mRequest != null) {
                        mRequest.finish();
                        updateDownloadFailed(DownloadManager.ERROR_DOWNLOAD_CANCELLED, "Téléchargement annulé");
                        mTimer.cancel();
                    }
                    return;
                }
    ...
    private void executeDownload(String downloadUrl) {
        ...
        try {
            ...
            updateDownloadFailed(DownloadManager.ERROR_MALFORMED_URI,"URL mal formée");
            return;
        }
        ...
        try {
            ...
            switch (responseCode) {
                case HTTP_OK:
                    ...
                    if (readResponseHeaders(conn) == 1) {
                        ...
                        updateDownloadFailed(DownloadManager.ERROR_DOWNLOAD_SIZE_UNKNOWN, "Impossible de récupérer la taille du fichier");
                    }
                    return;
                ...
                case HTTP_TEMP_REDIRECT:
                    ...
                    if (mRedirectionCount > MAX_REDIRECTS) {
                        updateDownloadFailed(DownloadManager.ERROR_TOO_MANY_REDIRECTS, "Trop de redirection");
                        return;
                    }
                    break;
            ...
            e.printStackTrace();
            updateDownloadFailed(DownloadManager.ERROR_HTTP_DATA_ERROR, "Problème avec des sockets bas-niveau");
        } finally{
            if (conn != null) {
                conn.disconnect();
            }
    ...
    private void transferData(HttpURLConnection conn) {
        ...
        try {
            ...
            try {
            	...
            	e.printStackTrace();
                updateDownloadFailed(DownloadManager.ERROR_FILE_ERROR, "Impossible d'écrire les fichiers");
            }
    ...
    private int readFromResponse( byte[] data, InputStream entityStream) {
        try {
            ...
            updateDownloadFailed(DownloadManager.ERROR_HTTP_DATA_ERROR, "Impossible de lire la réponse du serveur");
    ...
    private void attemptRetryOnTimeOutException()  {
        ...
        try {
            ...
            updateDownloadFailed(DownloadManager.ERROR_CONNECTION_TIMEOUT_AFTER_RETRIES,
                    "Impossible de joindre le serveur, essais dépassés");
        }
    }
